...
const ResearchMemberCard: FC<{
  ...
  return (
    <div
      className={cn(
        "group relative rounded-xl overflow-hidden bg-white border border-line hover:border-primary-300 hover:shadow-md hover:-translate-y-1 transition-all duration-300",
    ...
    >
      {/* Photo */}
      <div className="relative w-full aspect-square bg-gradient-to-br from-navy to-navy-deep overflow-hidden">
        {image ? (
          <Image
            src={image}
            alt={name}
          ...
          <div className="w-full h-full flex items-center justify-center">
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary-500 to-primary-300 flex items-center justify-center">
              <span className="font-hanken text-lg font-bold text-white">{initials}</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-4 space-y-2">
        {/* Name */}
        <h4 className="font-serif text-paragraph-md-semibold text-navy leading-tight">
          {name}
        </h4>

        {/* Role badge */}
        <div className="inline-flex">
          <span className="font-hanken text-xs font-semibold text-primary-600 bg-primary-50 px-2 py-1 rounded-full">
            {role}
          </span>
        </div>

        {/* Affiliation */}
        <p className="font-hanken text-paragraph-sm-regular text-secondary-500 line-clamp-2">
          {affiliation}
        </p>
          ...
          <div className="flex gap-2 pt-3">
            {links.github && (
              <a
                ...
                className="p-1.5 rounded-md bg-surface-secondary border border-line text-secondary-600 hover:bg-primary-50 hover:text-primary-600 hover:border-primary-300 transition-colors duration-200"
                title="GitHub"
              >
                <Github size={16} />
              </a>
